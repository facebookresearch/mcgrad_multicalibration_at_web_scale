...
class Dataset:
    def __init__(
        ...
        if verbose:
            print(groups_info_str(self.X, self.y, self.groups, self.group_names))
    ...
    def train_calibration_split(self, frac, train_overlap=0, seed=43):
        ...
        (
            ...
            self.train_idxs, self.groups, int(len(self.train_idxs) * frac), seed=seed
        )
        ...
        if train_overlap > 0:
            print(
                f"Introducing overlap between train / calibration data: {train_overlap:.2f}"
            )

            # draw some {train_overlap}-fraction of training data
            np.random.seed(seed)
            num_samples = int(len(self.train_idxs_no_calib) * train_overlap)
            tc_idxs = np.random.choice(
                self.train_idxs_no_calib, num_samples, replace=False
            )

            # update calib set
            # note that reindex_group assumes sorted indices, so sort here
            self.calibration_idxs = sorted(
                np.concatenate([self.calibration_idxs, tc_idxs])
            )
            self.X_calib = self.X[self.calibration_idxs]
            self.y_calib = self.y[self.calibration_idxs]
        ...
        return (
            self.X_train_no_calib,
            self.y_train_no_calib,
            self.groups_train_no_calib,
            self.X_calib,
            self.y_calib,
            self.groups_calib,
    ...
    def _preprocess_data(self):
        ...
        if (
            self.train_test_val_split["test"]
            + self.train_test_val_split["val"]
            + self.train_test_val_split["train"]
            > 1
        ):
        ...
        self.n = len(self.X)
        (self.test_idxs, self.groups_test, train_val_idxs, _) = split_data(
            np.arange(self.n),
            self.groups,
            int(self.n * self.test_size),
            self.test_split_seed,
        )
        self.X_test = self.X[self.test_idxs]
        self.y_test = self.y[self.test_idxs]

        (self.val_idxs, self.groups_val, self.train_idxs, self.groups_train) = (
            split_data(
                train_val_idxs,
                self.groups,
                int(self.n * self.val_size),
                self.val_split_seed,
            )
        )
    ...
    def _smote(self):
        ...
        sm = imbl_smote()
        balanced_idxs, _ = sm.fit_resample(
            np.arange(len(self.y_train)).reshape(-1, 1), self.y_train
        )
        ...
        groups_train_balanced = [
            reindex_group(balanced_idxs.flatten(), group) for group in self.groups_train
        ]
        ...
        max_len = max(
            len(orig_prop_0_str),
            len(orig_prop_1_str),
            len(smote_prop_0_str),
            len(smote_prop_1_str),
        )
        # Print with aligned arrows
        print(
            f"{space}{'Class 0:':<20} (orig) {orig_prop_0_str:>{max_len}} -> (SMOTE) {smote_prop_0_str:>{max_len}}"
        )
        print(
            f"{space}{'Class 1:':<20} (orig) {orig_prop_1_str:>{max_len}} -> (SMOTE) {smote_prop_1_str:>{max_len}}"
        )
        print(
            f"{space}{'# samples:':<20} (orig) {lbl_tot:>{max_len}} -> (SMOTE) {smt_lbl_tot}"
        )
        print("*" * 20)
...
def groups_info_df(X, y, groups, group_names):
    ...
    for i, group in enumerate(groups):
        ...
        if size == 0:
            ...
            y_mean = np.mean(y[group])
        data.append([group_names[i], size, f"{frac:.4f}", f"{y_mean:.4f}"])

    data.append(
        ["Dataset", N_samples, f"{N_samples / len(X):.4f}", f"{np.mean(y):.4f}"]
    )
