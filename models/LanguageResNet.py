...
class _LanguageResNet(nn.Module):
    ...
    def load_resnet(self, config):
        ...
        if resnet_type not in resnet_types_dic:
            print(
                "Error: 'resnet_type' should be in: ['20', '32', '44', '56', '110', '1202']"
            )
    ...
    def forward(self, ids):
        ...
        if self.stack_embedding:
            ...
            split = x.shape[-1] // 3
            x = torch.stack(
                [x[..., 0:split], x[..., split : 2 * split], x[..., 2 * split :]], dim=1
            )
        else:
...
class LanguageResNet:
    def __init__(
        ...
        (self.vocab, self.tokenizer, self.max_token_len) = (
            self.net.vocab,
            config["tokenizer"],
            config["max_token_len"],
        )
    ...
    def train(self, X_train, y_train, groups_train, X_val, y_val, groups_val):
        ...
        train_dataset = LanguageDatasetWrapper(
            X_train, y_train, config=self.transform_config
        )
        ...
        ece = CalibrationError(task="multiclass", num_classes=2, n_bins=10).to(
            self.device
        )
        ...
        if self.from_saved:
            if self.saved_epoch >= self.epochs - 1:
                raise ValueError(
                    "Model already trained for", self.saved_epoch, "epochs."
                )
        ...
        for epoch in epoch_range:
            ...
            for i, data in enumerate(train_dataloader):
                ...
                if i % 100 == 0:  # print every 1000 mini-batches
                    ...
                    print(
                        "[%d, %5d] loss: %.3f LR: %.5f running acc: %.5f running ECE %.5f"
                        % (
                            epoch + 1,
                            i + 1,
                            running_loss / 100,
            ...
            if epoch % self.val_eval_epoch == 0:
                ...
                with torch.no_grad():
                    ...
                    print(
                        "val: y-pred mean",
                        round(torch.mean(y_pred.type(torch.float32)).item(), 3),
                    )
                    print(
                        "val: y-true mean",
                        round(torch.mean(y_val.type(torch.float32)).item(), 3),
                    )
    ...
    def save_model(self, epoch, val_acc, old_best_val_acc):
        ...
        # best-val-acc: save only if val_acc is better than old_best_val_acc
            ...
            if val_acc > old_best_val_acc:
                ...
                shutil.copy(
                    self.SAVE_DIR + f"model_{epoch - self.val_eval_epoch}.pt",
                    self.SAVE_DIR + f"model_{epoch}.pt",
                )
    ...
    def predict_proba(self, X_text, with_logits=False):
        ...
        if with_logits:
            return confs.cpu().numpy(), logits.cpu().numpy()
        else:
