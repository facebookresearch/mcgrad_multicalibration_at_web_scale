...
def ecce_perc(f, y):
    ...
    raw_kuiper = kuiper_calibration_per_segment(
        labels=y, predicted_scores=f, normalization_method=None
    )[0]
...
def ecce_sigma(f, y):
    ...
    return kuiper_calibration_per_segment(
        labels=y, predicted_scores=f, normalization_method="kuiper_standard_deviation"
    )[0]
...
def subgroup_metrics(
    ...
    for i, group in enumerate(subgroups):
        ...
        group_prauc = skmetrics.average_precision_score(
            subgroup_targets, subgroup_confs
        )
    ...
    agg_metrics = {
        ...
        "prauc": round(
            skmetrics.average_precision_score(targets, positive_class_confs), 4
        ),
        "ECE": round(binnedECE(positive_class_confs, targets), 4),
        "smECE": round(smECE(positive_class_confs, targets), 4),
        "ECCE_perc": round(mce.global_ecce * mce.prevalence / 100, 4),
        "ECCE_sigma": round(mce.global_ecce_sigma_scale, 4),
        "MCE_perc": round(mce.mce_absolute, 4),
    ...
    def _agg_metric(agg_func, metric_name, sg_metrics):
        return round(
            agg_func(
                [
                    sg_metrics[i][metric_name]
                    for i in sg_metrics
                    if sg_metrics[i][metric_name] != np.nan
                ]
            ),
            4,
        )
    ...
    sg_mean = {
        metric_name: _agg_metric(np.mean, metric_name, subgroup_metrics)
    ...
    # add subgroup max
    sg_max = {
        metric_name: _agg_metric(np.max, metric_name, subgroup_metrics)
        for metric_name in all_metrics
    }
    ...
    # add subgroup min
    sg_min = {
        metric_name: _agg_metric(np.min, metric_name, subgroup_metrics)
        for metric_name in all_metrics
    }
