...
from metrics import Logger, print_metrics, subgroup_metrics
...
class Experiment:
    def __init__(
        self, dataset, model, calib_frac, calib_train_overlap=0, calib_seed=50
    ):
        """
        Parameters
        ...
        self.X_val, self.y_val, self.groups_val = (
            self.dataset.X_val,
            self.dataset.y_val,
    ...
    def multicalibrate(self, alg_type, params):
        ...
        # Create a DataFrame from the calibration data if needed for CASMCBoost
        df_calib = None
        if alg_type == "CASMCBoost" and self.dataset.X_calib is not None:
            # Create a DataFrame from the calibration data
            # If the dataset has feature names, use them, otherwise use generic column names
            if hasattr(self.dataset, "feature_names"):
                df_calib = pd.DataFrame(
                    self.dataset.X_calib, columns=self.dataset.feature_names
                )
            else:
                df_calib = pd.DataFrame(
                    self.dataset.X_calib,
                    columns=[
                        f"feature_{i}" for i in range(self.dataset.X_calib.shape[1])
                    ],
                )
    ...
    def evaluate_val(self, with_rel_diagram=False):
        self.evaluate_model(
            self.X_val, self.y_val, self.groups_val, "validation", with_rel_diagram
        )

    def evaluate_test(self, with_rel_diagram=False):
        self.evaluate_model(
            self.X_test, self.y_test, self.groups_test, "test", with_rel_diagram
        )

    def evaluate_train(self, with_rel_diagram=False):
        self.evaluate_model(
            self.X_train, self.y_train, self.groups_train, "train", with_rel_diagram
        )
    ...
    def evaluate_model(self, X, y, groups, dataset_split_name, with_rel_diagram=False):
        ...
        # log metrics
        if self.wandb:
            self.logger.log("ERM", dataset_split_name, original_model_metrics_val)
        print_metrics(
            original_model_metrics_val,
            algorithm=self.model.name,
        ...
        for mcbp, alg_type, mcb_params in self.mcb_models:
            # Create a DataFrame from the evaluation data if needed for CASMCBoost
            df_eval = None
            if alg_type == "CASMCBoost" and X is not None:
                # Create a DataFrame from the evaluation data
                # If the dataset has feature names, use them, otherwise use generic column names
                if hasattr(self.dataset, "feature_names"):
                    df_eval = pd.DataFrame(X, columns=self.dataset.feature_names)
                else:
                    df_eval = pd.DataFrame(
                        X,
                        columns=[
                            f"feature_{i}" for i in range(self.dataset.X.shape[1])
                        ],
                    )
            ...
            if alg_type == "Temp":
                mcb_confs = mcbp.batch_predict(logits, groups, df=df_eval)
            else:
                mcb_confs = mcbp.batch_predict(confs, groups, df=df_eval)
            ...
            if self.wandb:
                if alg_type == "HKRR":
                    self.logger.log(
                        f"{alg_type}_L{mcb_params['lambda']}_A{mcb_params['alpha']}",
                        dataset_split_name,
                        mcb_metrics,
                    )
                elif alg_type == "HJZ":
                    self.logger.log(
                        f"{alg_type}_{mcb_params['algorithm']}"
                    ...
                    self.logger.log(
                        f"{alg_type}_T{mcb_params['temperature']}",
                        dataset_split_name,
                        mcb_metrics,
                    )
            ...
            if with_rel_diagram:
                ...
                dir = (
                    f"plots/{self.model.name}/{self.dataset.name}/cf={self.calib_frac}"
                )
                os.makedirs(dir, exist_ok=True)
